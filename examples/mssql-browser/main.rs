...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match &mode[..] {
        "broadcast" => {
            ...
            let mut iterator = browser::browse(multicast)
                .await?;
        ...
        "host" => {
            ...
            let mut iterator = browser::browse_host(remote_addr).await?;
        ...
        "instance" => {
            ...
            let x = browser::browse_instance(remote_addr, &instance_name).await?;
            println!("Got response: {:#?}", x);
        ...
        "instance_dac" => {
            ...
            let x = browser::browse_instance_dac(remote_addr, &instance_name).await?;
            println!("Got response: {:#?}", x);
